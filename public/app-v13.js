...
async function generateVisualShopTurn() {
  ...
  try {
    ...
    setShopTurnStatus(
      `Готово: actual-blank→drawing→geometry→validation→setups→ShopTurn${review}. Пустые/жёлтые поля требуют проверки.`,
      "ready"
    );
  } catch (error) {
...
async function saveVisualShopTurn() {
  ...
  try {
    ...
    setShopTurnStatus("Проект v13 сохранён в Railway Postgres.", "ready");
    await loadShopTurnProjects();
  } catch (error) {
  ...
  document.getElementById("st_confirm_header")?.addEventListener("click", () => {
    ...
    setShopTurnStatus(
      shopTurnPlan.program.headerConfirmed
        ? "Фактическая заготовка подтверждена оператором. При следующей генерации GPT сохранит эти значения."
